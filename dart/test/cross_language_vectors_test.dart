...
void main() {
  group('Cross-language cork vectors', () {
    ...
    setUp(() async {
      expected = await _buildExpectedVectors();
    });
    ...
    test('vectors verify with expected outcomes', () async {
      ...
      final signers = (fixture['signers'] as Map<String, dynamic>).map(
        (key, value) => MapEntry(key, (
          base64Decode((value as Map<String, dynamic>)['keyId'] as String),
          base64Decode(value['masterKey'] as String),
        )),
      );

      final vectors = fixture['vectors'] as List<dynamic>;
      for (final entry in vectors.cast<Map<String, dynamic>>()) {
        final signerName = entry['signer'] as String;
        final signerTuple = signers[signerName];
        expect(signerTuple, isNotNull, reason: 'Signer $signerName missing');

        final (keyId, masterKey) = signerTuple!;
        final signer = Signer(keyId, masterKey);

        final token = entry['token'] as String;
        final cork = Cork.parse(token);

        final tailSignature = entry['tailSignature'] as Map<String, dynamic>;
        final actualSignature = base64Encode(cork.tailSignature);
        expect(actualSignature, equals(tailSignature['actual']));

        final expectedSignature = base64Encode(
          await computeTailSignature(cork.toProto(), signer),
        );
        expect(expectedSignature, equals(tailSignature['expected']));

        final expectSignatureValid =
            (entry['expect'] as Map<String, dynamic>)['signatureValid'] as bool;
        if (expectSignatureValid) {
          await expectLater(cork.verify(signer), completes);
        } else {
          await expectLater(
            cork.verify(signer),
        ...
        final expectedProto = entry['proto'] as Map<String, dynamic>;
        final actualProto =
            jsonDecode(
                  jsonEncode(
                    cork.toProto().toProto3Json(typeRegistry: typeRegistry),
                  ),
                )
